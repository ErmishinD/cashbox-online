<template>
	<div class="tac content_title">
		{{shop.name}}
		<small><router-link :to="{name: 'shops_edit', params: {ids: shop.id}}">{{ $t('Редактировать') }}</router-link></small>
	</div>
	<div class=" col-12">
			...
			<vue-good-table style="position: static;"
		      ...
		      <template #table-row="props">
		          <span class="table_actions" v-if="props.column.field == 'actions'">
		            <router-link :to="{name: 'storages_show', params: {id: props.row.shop_id}}"><i class="fas fa-eye"></i></router-link>
		          </span>
		        </template>
		    </vue-good-table>
		</div>
